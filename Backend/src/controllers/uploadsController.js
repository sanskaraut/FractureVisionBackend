...
import { adminSupabase, bucketName } from '../supabase.js';
...
function storagePathFor(kind, userId, filename) {
  const ts = Date.now();
  return `${kind}/${userId}/${ts}_${filename}`;
}
...
async function uploadToStorage(localPath, storagePath, contentType) {
  ...
  const { data: pub } = adminSupabase.storage.from(bucketName).getPublicUrl(storagePath);
...
export const uploadHandler = async (req, res) => {
  ...
  try {
    ...
    const imagePath = storagePathFor('images', userId, req.file.originalname.replace(/\s+/g, '_'));
    ...
    const modelPathInBucket = storagePathFor('models', userId, modelFileName);
...
export const listHistory = async (req, res) => {
  ...
  try {
    let query = adminSupabase.from('uploads').select('*').eq('user_id', userId).order('created_at', { ascending: false });
...
export const renameUpload = async (req, res) => {
  ...
  try {
    const { data: row, error: e0 } = await adminSupabase.from('uploads').select('user_id').eq('id', id).single();
    if (e0 || !row) return res.status(404).json({ error: 'Not found' });
    if (row.user_id !== userId) return res.status(403).json({ error: 'Forbidden' });

    const { data, error } = await adminSupabase.from('uploads').update({ name: newName }).eq('id', id).select().single();
    if (error) throw error;
...
export const deleteUpload = async (req, res) => {
  ...
  try {
    const { data: row, error: e0 } = await adminSupabase
      .from('uploads')
    ...
    // delete files
    await adminSupabase.storage.from(bucketName).remove([row.image_path, row.model_path]);

    // delete row
    const { error } = await adminSupabase.from('uploads').delete().eq('id', id);
    if (error) throw error;
